...
export class ProductsPageComponent implements OnInit {
  ...
  exportProducts(): void {
    ...
    from(this.productsApi.apiProductsExportGet$Response()).subscribe({
      ...
      error: () => {
        this.isExporting.set(false);
        this.snackBar.open(this.translate.instant('products.failedToExport'), this.translate.instant('common.close'), { duration: 4000 });
      }
    });
  }
  ...
  importProducts(fileInput: HTMLInputElement): void {
    ...
    from(this.productsApi.apiProductsImportPost$Json({
      ...
      error: () => {
        this.isImporting.set(false);
        fileInput.value = '';
        this.snackBar.open(this.translate.instant('products.failedToImport'), this.translate.instant('common.close'), { duration: 4000 });
      }
    });
  }
  ...
  downloadOrderPdf(product: Product): void {
    ...
    from(this.productsApi.apiProductsIdOrderPdfGet$Response({ id: product.id })).subscribe({
      ...
      error: () => {
        this.downloadingProductId.set(null);
        this.snackBar.open(this.translate.instant('products.failedToDownloadPdf'), this.translate.instant('common.close'), { duration: 4000 });
      }
    });
  }
